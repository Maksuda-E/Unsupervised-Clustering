...
with center_col:
    ...
    with form_col:
        ...
        if "mall_cluster_result" in st.session_state:
            ...
            #  prepares the prediction result html
            result_html = f"""
...
"""
        ...
        with st.container():
            ...
            st.markdown('<div class="guide-title">Segment Meaning Guide</div>', unsafe_allow_html=True)

            st.markdown(f'<div class="guide-item"><b>Cluster 0:</b> {segment_labels.get(0, "No description available.")}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="guide-item"><b>Cluster 1:</b> {segment_labels.get(1, "No description available.")}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="guide-item"><b>Cluster 2:</b> {segment_labels.get(2, "No description available.")}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="guide-item"><b>Cluster 3:</b> {segment_labels.get(3, "No description available.")}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="guide-item"><b>Cluster 4:</b> {segment_labels.get(4, "No description available.")}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="guide-item"><b>Cluster 5:</b> {segment_labels.get(5, "No description available.")}</div>', unsafe_allow_html=True)

            st.markdown(
                '<div class="guide-note">These descriptions are general cluster interpretations based on the trained clustering model.</div>',
                unsafe_allow_html=True
            )
